...
public struct UserDefaultsPreferencesStore: DiningPreferencesStore, @unchecked Sendable {
    private let defaults: UserDefaults
    private let key = "ezypick.diningPreferences"

    public init(defaults: UserDefaults = .standard) { self.defaults = defaults }

    public func load() -> DiningPreferences? {
        guard let data = defaults.data(forKey: key) else { return nil }
        return try? JSONDecoder().decode(DiningPreferences.self, from: data)
    }

    public func save(_ preferences: DiningPreferences) throws {
